...
The bit value "1" is encoded with a time interval of $\boldsymbol{T_1}$, while the bit value "0" is encoded with a time interval of $\boldsymbol{T_0}$. Naturally, due to potential timing discrepancies between the transmitter and the receiver, upper and lower bounds for the encoded values are defined (\textbf{$\pm N (%)$}).
